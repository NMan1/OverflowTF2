...
struct studio_data;
struct studio_decal_handle;
class studiohdr_t;
class IClientRenderable;
struct draw_model_state
{
	studiohdr_t* m_pStudioHdr;
	studio_data* m_pStudioData;
	IClientRenderable* m_pRenderable;
	const matrix3x4* m_pModelToWorld;
	studio_decal_handle* m_hDecals;
	int					 m_DrawFlags;
	int					 m_Lod;
};
...
public:
	...
	virtual preview_image_ret_val GetPreviewImageProperties(int* width, int* height,
		image_format* imageFormat, bool* isTranslucent) const = 0;

	virtual preview_image_ret_val GetPreviewImage(unsigned char* data,
	...
	virtual i_material* CheckProxyReplacement(void* proxyData) = 0;
